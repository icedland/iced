...
import com.github.icedland.iced.x86.internal.fmt.FormatterFlowControl;
...
public final class MasmFormatter extends Formatter {
	/**
	 ...
	 */
	public MasmFormatter(SymbolResolver symbolResolver) {
	...
	private static final FormatterString str_hnt = new FormatterString("hnt");
	private static final FormatterString str_ht = new FormatterString("ht");
	private static final FormatterString str_lock = new FormatterString("lock");
	private static final FormatterString str_near = new FormatterString("near");
	private static final FormatterString str_notrack = new FormatterString("notrack");
	private static final FormatterString str_offset = new FormatterString("offset");
	private static final FormatterString str_ptr = new FormatterString("ptr");
	...
	private static final FormatterString str_short = new FormatterString("short");
	...
	private boolean showSegmentPrefix(Instruction instruction, InstrOpInfo opInfo) {
		...
		for (int i = 0; i < opInfo.opCount; i++) {
			...
			case InstrOpKind.EXTRA_IMMEDIATE8_VALUE3:
	...
	private void formatOperand(Instruction instruction, FormatterOutput output, InstrOpInfo opInfo, int operand) {
		...
		case InstrOpKind.DECLARE_BYTE:
			...
			else if (opKind == InstrOpKind.EXTRA_IMMEDIATE8_VALUE3)
				imm8 = 3;
	...
	private void formatMemory(FormatterOutput output, Instruction instruction, int operand, int instructionOperand, int segReg, int baseReg,
		...
		int codeSize;
		boolean is1632 = ((codeSize = instruction.getCodeSize()) == CodeSize.CODE16 || codeSize == CodeSize.CODE32);
		boolean hasMemReg = baseReg != Register.NONE || indexReg != Register.NONE;
		boolean displInBrackets = symbol != null ? options.getMasmSymbolDisplInBrackets() : options.getMasmDisplInBrackets();
		int segOverride = instruction.getSegmentPrefix();
		if ((!is1632 && !hasMemReg && symbol == null)
				|| (is1632 && !hasMemReg && symbol == null && !options.getMasmAddDsPrefix32() && segOverride == Register.NONE))
			displInBrackets = true;
		boolean needBrackets = hasMemReg || displInBrackets;

		formatMemorySize(output, instruction, symbol, instruction.getMemorySize(), flags, operandOptions);
		...
		if (options.getAlwaysShowSegmentRegister()
				|| (segOverride != Register.NONE && !noTrackPrefix
						&& com.github.icedland.iced.x86.internal.fmt.FormatterUtils.showSegmentPrefix(Register.NONE, instruction, options))
				||
				(is1632 && !hasMemReg && symbol == null && options.getMasmAddDsPrefix32())) {
	...
	private void formatMemorySize(FormatterOutput output, Instruction instruction, SymbolResult symbol, int memSize, int flags,
			FormatterOperandOptions operandOptions) {
		int memSizeOptions = operandOptions.getMemorySizeOptions();
		if (memSizeOptions == MemorySizeOptions.NEVER)
			return;

		int memType = flags & InstrOpInfoFlags.MEM_SIZE_MASK;
		if (memType == InstrOpInfoFlags.MEM_SIZE_NOTHING)
			return;

		assert Integer.compareUnsigned(memSize, allMemorySizes.length) < 0 : memSize;
		MemorySizes.Info memInfo = allMemorySizes[memSize];
		FormatterString[] memSizeStrings = memInfo.keywords;

		switch (memInfo.size) {
		case 0:
			if (memType == InstrOpInfoFlags.MEM_SIZE_DWORD_OR_QWORD) {
		...
		case 8:
			if (memType == InstrOpInfoFlags.MEM_SIZE_MMX)
				memSizeStrings = MemorySizes.mmword_ptr;
			break;

		case 16:
			if (memType == InstrOpInfoFlags.MEM_SIZE_NORMAL)
				memSizeStrings = MemorySizes.oword_ptr;
			break;
		}

		if (memType == InstrOpInfoFlags.MEM_SIZE_XMMWORD_PTR)
		...
		if (memSizeOptions == MemorySizeOptions.DEFAULT) {
			...
			else if ((flags & InstrOpInfoFlags.SHOW_NO_MEM_SIZE_FORCE_SIZE) == 0 && !memInfo.isBroadcast)
				return;
		}
			...
			if ((flags & InstrOpInfoFlags.SHOW_MIN_MEM_SIZE_FORCE_SIZE) == 0 && !memInfo.isBroadcast)
				return;
		}
		else
			assert memSizeOptions == MemorySizeOptions.ALWAYS : memSizeOptions;

		for (FormatterString name : memSizeStrings) {
			formatKeyword(output, name);
			output.write(" ", FormatterTextKind.TEXT);
		}
	}

	private boolean isSameMemSize(FormatterString[] memSizeStrings, boolean isBroadcast, SymbolResult symbol) {
		if (isBroadcast)
			return false;
	...
	private void formatFlowControl(FormatterOutput output, int kind, FormatterOperandOptions operandOptions) {
		if (!operandOptions.getBranchSize())
			return;

		switch (kind) {
		case FormatterFlowControl.ALWAYS_SHORT_BRANCH:
			break;

		case FormatterFlowControl.SHORT_BRANCH:
			formatKeyword(output, str_short);
			output.write(" ", FormatterTextKind.TEXT);
			break;

		case FormatterFlowControl.NEAR_BRANCH:
		...
		case FormatterFlowControl.NEAR_CALL:
			break;

		case FormatterFlowControl.FAR_BRANCH:
		case FormatterFlowControl.FAR_CALL:
