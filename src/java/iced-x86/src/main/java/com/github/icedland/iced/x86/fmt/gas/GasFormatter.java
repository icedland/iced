...
public final class GasFormatter extends Formatter {
	/**
	...
	private FormatterString[] getAllRegisters() {
		return options.getGasNakedRegisters() ? allRegistersNaked : allRegisters;
	}
	...
	public GasFormatter(SymbolResolver symbolResolver, FormatterOptionsProvider optionsProvider) {
		this.options = com.github.icedland.iced.x86.internal.fmt.FormatterOptionsExt.createGas();
		...
		if ((mnemonicOptions & FormatMnemonicOptions.NO_PREFIXES) == 0 && (opInfo.flags & InstrOpInfoFlags.MNEMONIC_IS_DIRECTIVE) == 0) {
			...
			if ((opInfo.flags & InstrOpInfoFlags.OP_SIZE_IS_BYTE_DIRECTIVE) != 0) {
				...
				case SizeOverride.SIZE32:
					...
					output.write(" ", FormatterTextKind.TEXT);
					NumberFormattingOptions numberOptions = NumberFormattingOptions.createImmediate(options);
					String s = numberFormatter.formatUInt8(options, numberOptions, (byte)0x66);
				...
				case SizeOverride.SIZE64:
					column = formatPrefix(output, instruction, column, str_rex_w, PrefixKind.OPERAND_SIZE);
					break;
			...
			if (!hasNoTrackPrefix && prefixSeg != Register.NONE && showSegmentPrefix(instruction, opInfo))
				column = formatPrefix(output, instruction, column, allRegistersNaked[prefixSeg],
	...
	private void formatOperand(Instruction instruction, FormatterOutput output, InstrOpInfo opInfo, int operand) {
		...
		case InstrOpKind.IMMEDIATE8_2ND:
		case InstrOpKind.DECLARE_BYTE:
			if (opKind != InstrOpKind.DECLARE_BYTE)
				output.write(IMMEDIATE_VALUE_PREFIX, FormatterTextKind.OPERATOR);
		...
		case InstrOpKind.IMMEDIATE8TO16:
		case InstrOpKind.DECLARE_WORD:
			if (opKind != InstrOpKind.DECLARE_WORD)
				output.write(IMMEDIATE_VALUE_PREFIX, FormatterTextKind.OPERATOR);
		...
		case InstrOpKind.IMMEDIATE8TO32:
		case InstrOpKind.DECLARE_DWORD:
			if (opKind != InstrOpKind.DECLARE_DWORD)
				output.write(IMMEDIATE_VALUE_PREFIX, FormatterTextKind.OPERATOR);
		...
		case InstrOpKind.IMMEDIATE32TO64:
		case InstrOpKind.DECLARE_QWORD:
			if (opKind != InstrOpKind.DECLARE_QWORD)
				output.write(IMMEDIATE_VALUE_PREFIX, FormatterTextKind.OPERATOR);
	...
	private String toRegisterString(int reg) {
		assert Integer.compareUnsigned(reg, getAllRegisters().length) < 0 : reg;
		if (options.getPreferST0() && reg == Registers.REGISTER_ST)
			reg = Register.ST0;
		FormatterString regStr = getAllRegisters()[reg];
	...
	private void formatMemory(FormatterOutput output, Instruction instruction, int operand, int instructionOperand, int segReg, int baseReg,
			int indexReg, int scale, int displSize, long displ, int addrSize) {
		...
		boolean hasBaseOrIndexReg = baseReg != Register.NONE || indexReg != Register.NONE;
		...
		if (symbol != null)
			com.github.icedland.iced.x86.internal.fmt.FormatterOutputExt.write(output, instruction, operand, instructionOperand, options,
					numberFormatter, numberOptions, absAddr, symbol, options.getShowSymbolAddress());
		...
		assert Integer.compareUnsigned(memSize, allMemorySizes.length) < 0;
		FormatterString bcstTo = allMemorySizes[memSize];
