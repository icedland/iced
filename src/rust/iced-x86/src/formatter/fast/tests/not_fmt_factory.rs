...
use crate::formatter::fast::tests::not_fast_fmt::NotFastFormatter;
use crate::formatter::SymbolResolver;
use alloc::boxed::Box;

pub(super) fn create_default() -> Box<NotFastFormatter> {
	Box::new(NotFastFormatter::new())
}

pub(super) fn create_inverted() -> Box<NotFastFormatter> {
	let mut fmt = NotFastFormatter::new();
...
pub(super) fn create_options() -> Box<NotFastFormatter> {
	let mut fmt = NotFastFormatter::new();
	fmt.options_mut().set_rip_relative_addresses(true);
	Box::new(fmt)
}

pub(super) fn create_resolver(symbol_resolver: Box<dyn SymbolResolver>) -> Box<NotFastFormatter> {
	let mut fmt = NotFastFormatter::try_with_options(Some(symbol_resolver)).unwrap();
	fmt.options_mut().set_rip_relative_addresses(true);
	Box::new(fmt)
}
