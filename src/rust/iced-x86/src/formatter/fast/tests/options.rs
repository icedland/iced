...
fn test_specialized_formatter_trait_options() {
	...
	for _ in 0..2 {
		assert_eq!(MySpecializedFormatterTraitOptions::ENABLE_SYMBOL_RESOLVER, false);
		assert_eq!(MySpecializedFormatterTraitOptions::SUPPORTS_DB_DW_DD_DQ, cfg!(feature = "db"));
		assert_eq!(unsafe { MySpecializedFormatterTraitOptions::verify_output_has_enough_bytes_left() }, true);
		assert_eq!(MySpecializedFormatterTraitOptions::space_after_operand_separator(options), false);
		assert_eq!(MySpecializedFormatterTraitOptions::rip_relative_addresses(options), true);
		assert_eq!(MySpecializedFormatterTraitOptions::use_pseudo_ops(options), false);
		assert_eq!(MySpecializedFormatterTraitOptions::show_symbol_address(options), false);
		assert_eq!(MySpecializedFormatterTraitOptions::always_show_segment_register(options), false);
		assert_eq!(MySpecializedFormatterTraitOptions::always_show_memory_size(options), false);
		assert_eq!(MySpecializedFormatterTraitOptions::uppercase_hex(options), true);
		assert_eq!(MySpecializedFormatterTraitOptions::use_hex_prefix(options), true);
...
fn test_default_specialized_formatter_trait_options() {
	type MySpecializedFormatter = SpecializedFormatter<DefaultSpecializedFormatterTraitOptions>;
	...
	for _ in 0..2 {
		...
		assert_eq!(DefaultSpecializedFormatterTraitOptions::SUPPORTS_DB_DW_DD_DQ, cfg!(feature = "db"));
...
fn test_default_fast_formatter_trait_options() {
	type MySpecializedFormatter = SpecializedFormatter<DefaultFastFormatterTraitOptions>;

	let mut formatter = MySpecializedFormatter::new();
	let options = formatter.options_mut();

	assert_eq!(DefaultFastFormatterTraitOptions::ENABLE_SYMBOL_RESOLVER, true);
	assert_eq!(DefaultFastFormatterTraitOptions::SUPPORTS_DB_DW_DD_DQ, true);
