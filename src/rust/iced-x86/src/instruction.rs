...
pub struct Instruction {
	...
	pub(crate) len: u8,
	pub(crate) legacy_prefix_count: u8,
	pub(crate) legacy_prefix_bytes: [u8; 8],
}
#[cfg(test)]
#[allow(dead_code)]
pub(crate) const INSTRUCTION_TOTAL_SIZE: usize = 48;
...
impl Instruction {
	...
	pub fn eq_all_bits(&self, other: &Self) -> bool {
		...
		// `legacy_prefix_bytes`/`legacy_prefix_count` are encoding metadata set only by
		// the decoder; they are intentionally excluded so that decoded instructions still
		// compare equal to those built with `Instruction::with_*()` constructors.
	...
	pub fn set_code(&mut self, new_value: Code) {
		// Clear recorded legacy prefix bytes when the instruction form changes after
		// initial decode, so stale bytes don't carry over into the re-encoded form.
		// The initial decode path goes INVALID → real code (prefix bytes are pushed
		// before set_code is called), so we skip clearing in that case.
		if new_value != self.code && self.code != Code::INVALID {
			self.legacy_prefix_count = 0;
			self.legacy_prefix_bytes = [0; 8];
		}
		self.code = new_value
	}
	...
	/// Clears any legacy prefix bytes recorded during decode, causing the encoder to
	/// recompute the canonical prefix sequence from instruction flags.  Call this
	/// when rewriting an instruction's addressing or operand form so that stale prefix
	/// bytes from the original encoding do not carry over.
	#[inline]
	pub fn clear_legacy_prefix_bytes(&mut self) {
		self.legacy_prefix_count = 0;
		self.legacy_prefix_bytes = [0; 8];
	}

	/// Returns the legacy prefix bytes recorded by the decoder in their original order.
	///
	/// When non-empty, the [`Encoder`] emits these bytes verbatim instead of computing
	/// the canonical prefix order from flags, allowing bit-identical round-tripping of
	/// instructions whose prefix ordering differs from the canonical form (e.g. `67 66 2E`
	/// instead of the canonical `2E 66 67`).
	///
	/// [`Encoder`]: struct.Encoder.html
	#[must_use]
	#[inline]
	pub fn legacy_prefix_bytes(&self) -> &[u8] {
		&self.legacy_prefix_bytes[..self.legacy_prefix_count as usize]
	}

	/// Records one legacy prefix byte (called by the decoder for each prefix byte seen).
	/// Silently ignores overflow beyond 4 entries.
	#[inline]
	pub(crate) fn push_legacy_prefix(&mut self, byte: u8) {
		let idx = self.legacy_prefix_count as usize;
		if idx < self.legacy_prefix_bytes.len() {
			self.legacy_prefix_bytes[idx] = byte;
			self.legacy_prefix_count += 1;
		}
	}
...
impl PartialEq<Instruction> for Instruction {
	...
	fn eq(&self, other: &Self) -> bool {
		self.mem_displ == other.mem_displ
			...
			&& self.scale == other.scale
			&& self.displ_size == other.displ_size
		// legacy_prefix_count/legacy_prefix_bytes are encoding metadata, not semantics;
		// excluded from == (like EQUALS_IGNORE_MASK bits) so that instructions decoded
		// from non-canonical prefix orderings still compare equal to their canonical forms.
	}
}
...
impl Hash for Instruction {
	...
	fn hash<H: Hasher>(&self, state: &mut H) {
		...
		// legacy_prefix_count/legacy_prefix_bytes excluded (encoding metadata only)
	}
}
