...
macro_rules! mk_to_enum {
	($name:ident, $enum_ty:ty, $name_str:literal) => {
		pub(super) unsafe fn $name(lua: &Lua<'_>, value: u32) -> $enum_ty {
			match <$enum_ty as TryFrom<usize>>::try_from(value as usize) {
				Ok(result) => result,
				Err(_) => unsafe { lua.throw_error_msg(concat!("Invalid ", $name_str, " value")) },
			}
		}
	};
}
