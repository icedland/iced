...
// Lua 5.4 `lua.h` converted to Rust
...
pub type lua_GetIType = lua_Integer;
...
pub const LUA_EXTRASPACE: usize = mem::size_of::<*mut c_void>();
...
pub type lua_KFunction = unsafe extern "C" fn(L: lua_State, status: c_int, ctx: lua_KContext) -> c_int;
...
pub type lua_WarnFunction = unsafe extern "C" fn(ud: *mut c_void, msg: *const c_char, tocont: c_int);
...
extern "C" {
	...
	pub fn lua_resetthread(L: lua_State) -> c_int;
	...
	pub fn lua_rotate(L: lua_State, idx: c_int, n: c_int);
	pub fn lua_copy(L: lua_State, fromidx: c_int, toidx: c_int);
	pub fn lua_checkstack(L: lua_State, n: c_int) -> c_int;
	...
	pub fn lua_isinteger(L: lua_State, idx: c_int) -> c_int;
	...
	pub fn lua_rawlen(L: lua_State, idx: c_int) -> lua_Unsigned;
...
extern "C" {
	...
	pub fn lua_pushlstring(L: lua_State, s: *const c_char, len: size_t) -> *const c_char;
	...
	pub fn lua_getglobal(L: lua_State, name: *const c_char) -> c_int;
	pub fn lua_gettable(L: lua_State, idx: c_int) -> c_int;
	pub fn lua_getfield(L: lua_State, idx: c_int, k: *const c_char) -> c_int;
	pub fn lua_geti(L: lua_State, idx: c_int, n: lua_GetIType) -> c_int;
	pub fn lua_rawget(L: lua_State, idx: c_int) -> c_int;
	pub fn lua_rawgeti(L: lua_State, idx: c_int, n: lua_GetIType) -> c_int;
	pub fn lua_rawgetp(L: lua_State, idx: c_int, p: *const c_void) -> c_int;

	pub fn lua_createtable(L: lua_State, narr: c_int, nrec: c_int);
	pub fn lua_newuserdatauv(L: lua_State, sz: size_t, nuvalue: c_int) -> *mut c_void;
	pub fn lua_getmetatable(L: lua_State, objindex: c_int) -> c_int;
	pub fn lua_getiuservalue(L: lua_State, idx: c_int, n: c_int) -> c_int;
	...
	pub fn lua_seti(L: lua_State, idx: c_int, n: lua_GetIType);
	...
	pub fn lua_setwarnf(L: lua_State, f: lua_WarnFunction, ud: *mut c_void);
...
pub unsafe fn lua_strlen(L: lua_State, i: c_int) -> size_t {
	unsafe { lua_rawlen(L, i) as size_t }
}
...
extern "C" {
	pub fn lua_gc(L: lua_State, what: c_int, ...) -> c_int;
	...
	pub fn lua_stringtonumber(L: lua_State, s: *const c_char) -> size_t;
...
#[inline]
pub unsafe fn lua_getextraspace(L: lua_State) -> *mut c_void {
	unsafe { L.cast::<c_char>().sub(LUA_EXTRASPACE).cast() }
}
...
pub unsafe fn lua_pushglobaltable(L: lua_State) {
	unsafe {
		let _ = lua_rawgeti(L, LUA_REGISTRYINDEX, LUA_RIDX_GLOBALS as lua_GetIType);
	}
}
...
#[inline]
pub unsafe fn lua_insert(L: lua_State, idx: c_int) {
	unsafe {
		lua_rotate(L, idx, 1);
	}
}
...
#[inline]
pub unsafe fn lua_remove(L: lua_State, idx: c_int) {
	unsafe {
		lua_rotate(L, idx, -1);
		lua_pop(L, 1);
	}
...
#[inline]
pub unsafe fn lua_replace(L: lua_State, idx: c_int) {
	unsafe {
		lua_copy(L, -1, idx);
		lua_pop(L, 1);
	}
...
#[inline]
pub unsafe fn lua_pushunsigned(L: lua_State, n: lua_Unsigned) {
	unsafe {
...
#[inline]
pub unsafe fn lua_tounsignedx(L: lua_State, i: c_int, is: *mut c_int) -> lua_Unsigned {
	unsafe { lua_tointegerx(L, i, is) as lua_Unsigned }
}
...
#[inline]
pub unsafe fn lua_newuserdata(L: lua_State, s: size_t) -> *mut c_void {
	unsafe { lua_newuserdatauv(L, s, 1) }
}

#[inline]
pub unsafe fn lua_getuservalue(L: lua_State, idx: c_int) -> c_int {
...
extern "C" {
	...
	pub fn lua_sethook(L: lua_State, func: lua_Hook, mask: c_int, count: c_int);
	pub fn lua_gethook(L: lua_State) -> lua_Hook;
	pub fn lua_gethookmask(L: lua_State) -> c_int;
	pub fn lua_gethookcount(L: lua_State) -> c_int;

	pub fn lua_setcstacklimit(L: lua_State, limit: c_uint) -> c_int;
}
...
pub unsafe fn luaL_getmetatable(L: lua_State, n: *const c_char) {
	unsafe {
		let _ = lua_getfield(L, LUA_REGISTRYINDEX, n);
	}
}
