...
#pragma once
#ifndef ICED_X86_MASM_FORMATTER_HPP
#define ICED_X86_MASM_FORMATTER_HPP
...
/// @brief MASM (Microsoft Macro Assembler) formatter
/// 
/// Formats instructions using MASM syntax (destination, source order).
/// Uses "type ptr" for memory size (e.g., "dword ptr").
/// Example: @c mov dword ptr [ebx+ecx*4+10h], eax
class MasmFormatter {
public:
  /// @brief Creates a new MASM formatter with default options
  MasmFormatter() = default;

  /// @brief Creates a new MASM formatter with the specified options
  /// @param options Formatter options
  explicit MasmFormatter( const FormatterOptions& options ) : options_( options ) {}

  /// @brief Creates a new MASM formatter with a symbol resolver
  /// @param symbol_resolver Symbol resolver (can be nullptr)
  explicit MasmFormatter( SymbolResolver* symbol_resolver )
      : symbol_resolver_( symbol_resolver ) {}

  /// @brief Creates a new MASM formatter with options and symbol resolver
  /// @param options Formatter options
  /// @param symbol_resolver Symbol resolver (can be nullptr)
  MasmFormatter( const FormatterOptions& options, SymbolResolver* symbol_resolver )
...
inline std::string_view MasmFormatter::format_register( Register reg ) const noexcept {
...
inline std::string MasmFormatter::format_to_string( const Instruction& instruction ) {
  std::string result;
...
inline void MasmFormatter::format( const Instruction& instruction, FormatterOutput& output ) {
  // Format prefixes
...
inline void MasmFormatter::format_mnemonic( const Instruction& instruction, FormatterOutput& output ) {
  std::string_view mnemonic = get_mnemonic( instruction.mnemonic() );
  output.write_mnemonic( instruction, mnemonic );
}

inline void MasmFormatter::format_operands( const Instruction& instruction, FormatterOutput& output ) {
  uint32_t op_count = instruction.op_count();
...
inline void MasmFormatter::format_operand( const Instruction& instruction, uint32_t operand,
                                            FormatterOutput& output ) {
...
inline void MasmFormatter::format_register_operand( const Instruction& instruction, uint32_t operand,
...
inline void MasmFormatter::format_immediate( const Instruction& instruction, uint32_t operand,
                                              FormatterOutput& output ) {
  OpKind kind = instruction.op_kind( operand );
  uint64_t value = 0;
...
inline void MasmFormatter::format_near_branch( const Instruction& instruction, uint32_t operand,
                                                FormatterOutput& output ) {
  OpKind kind = instruction.op_kind( operand );
  uint64_t target = 0;
  int addr_size = 4;
...
inline void MasmFormatter::format_far_branch( const Instruction& instruction, uint32_t operand,
                                               FormatterOutput& output ) {
  (void)operand;
...
inline void MasmFormatter::format_memory( const Instruction& instruction, uint32_t operand,
                                           FormatterOutput& output ) {
  OpKind kind = instruction.op_kind( operand );
...
inline void MasmFormatter::format_number( uint64_t value, FormatterOutput& output ) {
...
inline void MasmFormatter::format_signed_number( int64_t value, FormatterOutput& output ) {
  if ( value < 0 ) {
    output.write( "-", FormatterTextKind::OPERATOR );
...
inline void MasmFormatter::write_symbol( const Instruction& instruction, FormatterOutput& output,
...
inline void MasmFormatter::format_evex_decorators( const Instruction& instruction, uint32_t operand,
                                                    FormatterOutput& output ) {
  (void)operand;
...
inline std::string_view MasmFormatter::get_mnemonic( Mnemonic mnemonic ) const {
  bool uppercase = options_.uppercase_mnemonics() || options_.uppercase_all();
  return internal::get_mnemonic_string( mnemonic, uppercase );
}

inline std::string_view MasmFormatter::get_memory_size_string( const Instruction& instruction ) const {
...
#endif // ICED_X86_MASM_FORMATTER_HPP
