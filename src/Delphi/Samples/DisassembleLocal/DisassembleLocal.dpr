...
begin
  ...
  Decoder := TIcedDecoder.Create( EXAMPLE_CODE_BITNESS );

  {$IF CompilerVersion < 23}{$RANGECHECKS OFF}{$IFEND} // RangeCheck might cause Internal-Error C1118
  Decoder.SetData( @EXAMPLE_CODE[ 0 ], Length( EXAMPLE_CODE ), EXAMPLE_CODE_RIP, doNONE );
  ...
  Formatter := TIcedFormatter.Create( ftNasm );
  ...
  while Decoder.CanDecode do
    ...
    sOutput := Format( '%.16x ', [ instruction.RIP ] ) + sOutput + Formatter.FormatToString( Instruction );
