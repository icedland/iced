...
namespace Iced.UnitTests.Intel.DecoderTests {
	public sealed class DecoderTest_1_20_27 : DecoderTest {
		[Fact]
		void Test16_And_Eb_Gb_1() {
			var decoder = CreateDecoder16("20 CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.And_Eb_Gb, instr.Code);
		...
		void Test64_And_Eb_Gb_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
		...
		[Fact]
		void Test16_And_Ew_Gw_1() {
			var decoder = CreateDecoder16("21 CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.And_Ew_Gw, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(2, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_And_Ew_Gw_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_And_Ed_Gd_1() {
			var decoder = CreateDecoder16("66 21 CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.And_Ed_Gd, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(3, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_And_Ed_Gd_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		void Test64_And_Eq_Gq_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();

			Assert.Equal(Code.And_Eq_Gq, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(byteLength, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		[Fact]
		void Test16_And_Gb_Eb_1() {
			var decoder = CreateDecoder16("22 CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.And_Gb_Eb, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(2, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_And_Gb_Eb_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_And_Gw_Ew_1() {
			var decoder = CreateDecoder16("23 CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.And_Gw_Ew, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(2, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_And_Gw_Ew_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_And_Gd_Ed_1() {
			var decoder = CreateDecoder16("66 23 CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.And_Gd_Ed, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(3, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_And_Gd_Ed_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		void Test64_And_Gq_Eq_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();

			Assert.Equal(Code.And_Gq_Eq, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(byteLength, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		[Fact]
		void Test16_And_AL_Ib_1() {
			var decoder = CreateDecoder16("24 5A");
			var instr = decoder.Decode();

			Assert.Equal(Code.And_AL_Ib, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(2, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_And_AL_Ib_1(string hexBytes, int byteLength) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_And_AX_Iw_1() {
			var decoder = CreateDecoder16("25 5AA5");
			var instr = decoder.Decode();

			Assert.Equal(Code.And_AX_Iw, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(3, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_And_AX_Iw_1(string hexBytes, int byteLength) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_And_EAX_Id_1() {
			var decoder = CreateDecoder16("66 25 5AA51234");
			var instr = decoder.Decode();

			Assert.Equal(Code.And_EAX_Id, instr.Code);
			Assert.Equal(2, instr.OpCount);
		...
		void Test64_And_EAX_Id_1(string hexBytes, int byteLength) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		void Test64_And_RAX_Id64_1(string hexBytes, int byteLength) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();

			Assert.Equal(Code.And_RAX_Id64, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(byteLength, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
