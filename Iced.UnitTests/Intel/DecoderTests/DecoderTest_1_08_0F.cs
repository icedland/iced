...
namespace Iced.UnitTests.Intel.DecoderTests {
	public sealed class DecoderTest_1_08_0F : DecoderTest {
		[Fact]
		void Test16_Or_rm8_r8_1() {
			var decoder = CreateDecoder16("08 CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_rm8_r8, instr.Code);
		...
		void Test64_Or_rm8_r8_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
		...
		[Fact]
		void Test16_Or_rm16_r16_1() {
			var decoder = CreateDecoder16("09 CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_rm16_r16, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(2, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_Or_rm16_r16_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_Or_rm32_r32_1() {
			var decoder = CreateDecoder16("66 09 CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_rm32_r32, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(3, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_Or_rm32_r32_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		void Test64_Or_rm64_r64_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_rm64_r64, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(byteLength, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		[Fact]
		void Test16_Or_r8_rm8_1() {
			var decoder = CreateDecoder16("0A CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_r8_rm8, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(2, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_Or_r8_rm8_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_Or_r16_rm16_1() {
			var decoder = CreateDecoder16("0B CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_r16_rm16, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(2, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_Or_r16_rm16_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_Or_r32_rm32_1() {
			var decoder = CreateDecoder16("66 0B CE");
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_r32_rm32, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(3, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_Or_r32_rm32_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		void Test64_Or_r64_rm64_1(string hexBytes, int byteLength, Register reg1, Register reg2) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_r64_rm64, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(byteLength, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		[Fact]
		void Test16_Or_AL_imm8_1() {
			var decoder = CreateDecoder16("0C 5A");
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_AL_imm8, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(2, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_Or_AL_imm8_1(string hexBytes, int byteLength) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_Or_AX_imm16_1() {
			var decoder = CreateDecoder16("0D 5AA5");
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_AX_imm16, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(3, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
		...
		void Test64_Or_AX_imm16_1(string hexBytes, int byteLength) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		[Fact]
		void Test16_Or_EAX_imm32_1() {
			var decoder = CreateDecoder16("66 0D 5AA51234");
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_EAX_imm32, instr.Code);
			Assert.Equal(2, instr.OpCount);
		...
		void Test64_Or_EAX_imm32_1(string hexBytes, int byteLength) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();
		...
		void Test64_Or_RAX_imm32_1(string hexBytes, int byteLength) {
			var decoder = CreateDecoder64(hexBytes);
			var instr = decoder.Decode();

			Assert.Equal(Code.Or_RAX_imm32, instr.Code);
			Assert.Equal(2, instr.OpCount);
			Assert.Equal(byteLength, instr.ByteLength);
			Assert.False(instr.HasPrefixRepe);
			Assert.False(instr.HasPrefixRepne);
