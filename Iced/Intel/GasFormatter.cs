	...
	public sealed class GasFormatter : Formatter {
		...
		void FormatOperand(ref Instruction instruction, FormatterOutput output, ref InstrOpInfo opInfo, int operand) {
			...
			case InstrOpKind.Memory:
				...
				FormatMemory(output, ref instruction, operand, instruction.MemorySize, instruction.PrefixSegment, instruction.MemorySegment, baseReg, indexReg, instruction.MemoryIndexScale, displSize, displ, addrSize);
				break;
			...
			if (operand + 1 == opInfo.OpCount && instruction.HasOpMaskRegister) {
				output.Write("{", FormatterOutputTextKind.Punctuation);
